...
export function AutoTermsTable({ terms }: AutoTermsTableProps) {
      ...
      if (term.sources) {
        term.sources.forEach((source) => {
          if (source && typeof source === "object") {
            ...
            if (
              roleString.includes("Technology") ||
              roleString.includes("Tech")
            )
              sectorsSet.add("Technology");
            if (
              roleString.includes("Finance") ||
              roleString.includes("Financial")
            )
              sectorsSet.add("Finance");
            if (
              roleString.includes("Healthcare") ||
              roleString.includes("Health")
            )
              sectorsSet.add("Healthcare");
            if (roleString.includes("Education")) sectorsSet.add("Education");
            if (roleString.includes("Retail")) sectorsSet.add("Retail");
            if (roleString.includes("Manufacturing"))
              sectorsSet.add("Manufacturing");
            if (roleString.includes("Consulting")) sectorsSet.add("Consulting");
            if (
              roleString.includes("Media") ||
              roleString.includes("Entertainment")
            )
            ...
            if (roleString.includes("Intern"))
              experienceLevelsSet.add("Intern");
            if (
              roleString.includes("Early Career") ||
              roleString.includes("0-2 years")
            )
              experienceLevelsSet.add("Early Career (0-2 years)");
            if (
              roleString.includes("Mid-level") ||
              roleString.includes("3-5 years")
            )
              experienceLevelsSet.add("Mid-level (3-5 years)");
            if (
              roleString.includes("Senior") ||
              roleString.includes("6-10 years")
            )
              experienceLevelsSet.add("Senior (6-10 years)");
            if (
              roleString.includes("Staff") ||
              roleString.includes("Principal") ||
              roleString.includes("10+ years")
            )
              experienceLevelsSet.add("Staff/Principal (10+ years)");
            if (
              roleString.includes("Executive") ||
              roleString.includes("Leadership")
            )
        ...
        matchesSector =
          term.sources?.some(
            (source) =>
              source &&
              typeof source === "object" &&
              String(source.role).includes(sectorFilter)
          ) || false;
      }
        ...
        matchesExperience =
          term.sources?.some(
            (source) =>
              source &&
              typeof source === "object" &&
              String(source.role).includes(experienceFilter.split(" ")[0])
          ) || false;
      }
      ...
      <div className="flex flex-col space-y-4">
        <div className="flex flex-row gap-4">
          <div className="relative flex-1">
            ...
            <Input
              placeholder="Filter auto-generated terms..."
          ...
          <TableBody>
            {sortedTerms.length === 0 ? (
              <TableRow>
                ...
                >
                  No auto-generated results found.
                </TableCell>
              </TableRow>
            ) : (
              sortedTerms.map((term, index) => {
                const termKey = `auto-${term.term}-${index}`;
                                        ...
                                        term.sources.length > 0 ? (
                                          term.sources.map((source, idx) => (
                                            ...
                                            >
                                              <Badge
                                                variant="outline"
                                                className="px-3 py-1 bg-white/80 border-purple-300 text-slate-700 font-medium"
                                              >
                                                {source &&
                                                typeof source === "object"
